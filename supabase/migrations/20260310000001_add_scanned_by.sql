-- Add scanned_by column to track which staff member checked in the participant
ALTER TABLE public.event_registrations ADD COLUMN scanned_by UUID REFERENCES public.profiles(id) DEFAULT NULL;

-- Update the check-in RPC to accept p_scanned_by
CREATE OR REPLACE FUNCTION process_event_checkin(p_registration_id uuid, p_scanned_by uuid DEFAULT NULL)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
AS $$
